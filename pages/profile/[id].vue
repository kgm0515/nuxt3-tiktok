  ...
  <MainLayout>
    <div class="pt-[90px] 2xl:pl-[185px] lg:pl-[160px] lg:pr-0 pr-2 w-[calc(100%-90px)] max-w-[1800px] 2xl:mx-auto">
      <div class="flex w-[calc(100vw-230px)]">
        <img class="max-w-[120px] rounded-full" src="https:/picsum.photos/id/8/300/320" alt="" />
        <div class="ml-5 w-full">
          <div class="text-[30px] font-bold truncate">User name</div>
          <button v-if="true" class="flex items-center rounded-md py-1.5 px-3.5 mt-3 text-[15px] font-semibold border hover:bg-gray-100">
            <Icon class="mt-0.5 mr-1" name="mdi:pencil" size="18" />
            <div>Edit profile</div>
          </button>
          <button v-else class="flex items-center rounded-md py-1.5 px-8 mt-3 text-[15px] text-white font-semibold bg-[#f02c56]">Folow</button>
        </div>
      </div>
      ...
      <div class="pt-4 mr-4 text-gray-500 font-light text-[15px] pl-1.5 max-w-[500px]">This is the bio section</div>
      <div class="w-full flex item-center pt-4 border-b">
        <div class="w-60 text-center py-2 text-[17px] font-semibold border-b-2 border-b-black">Videos</div>
        <div class="w-60 text-center py-2 text-[17px] font-semibold text-gray-500"><Icon name="material-symbols:lock-open" class="mb-0.5" /> Liked</div>
      </div>
...
<script setup lang="ts">
  ...
  import PostUser from '@/components/pages/profile/PostUser.vue'
</script>
